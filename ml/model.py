...
FEATURE_RU = {
    "genre_encoded":     "жанр",
    "author_popularity": "популярность автора",
    "year_published":    "год издания",
    "price":             "цена",
    "page_count":        "кол-во страниц",
    "times_borrowed":    "число выдач",
    "rating":            "рейтинг",
}
...
def preprocess(df):
    ...
    features = [
        "genre_encoded", "author_popularity", "year_published",
...
def _plot_genre_popularity(df):
    ...
    df2 = df.copy()
    df2["genre_ru"] = df2["genre"].map(GENRE_RU).fillna(df2["genre"])
    genre_stats = (
        df2.groupby("genre_ru")["is_popular"]
        .mean()
    ...
    bars = plt.bar(
        genre_stats.index,
        genre_stats.values * 100,
        color=colors,
        edgecolor="white",
        linewidth=0.5
    )
    plt.title("доля популярных книг по жанрам (%)", fontsize=14, fontweight="bold", pad=15)
    plt.xlabel("жанр", fontsize=12)
    plt.ylabel("доля популярных книг (%)", fontsize=12)
...
def _plot_correlation(df):
    num_cols = [
        "author_popularity", "year_published", "price",
        "page_count", "times_borrowed", "rating", "is_popular"
    ]
    col_labels = [
        "попул.\nавтора", "год\nиздания", "цена",
        "стр.", "число\nвыдач", "рейтинг", "популярна"
    ]
    ...
    # без маски — показываем всю матрицу
    sns.heatmap(
        corr,
        annot=True,
        fmt=".2f",
...
def _plot_feature_importance(clf, features):
    ...
    sorted_features    = [FEATURE_RU.get(features[i], features[i]) for i in indices]
    ...
    plt.title("важность признаков", fontsize=14, fontweight="bold", pad=15)
...
def _plot_roc(y_test, y_proba):
    ...
    plt.ylabel("доля верных срабатываний", fontsize=12)
    plt.title("roc-кривая модели", fontsize=14, fontweight="bold", pad=15)
...
def predict_book(genre, author_popularity, year_published,
                 price, page_count, times_borrowed, rating):
    ...
    if prediction == 1:
        if probability > 0.85:
            recommendation = "рекомендуем закупить 10-15 экземпляров"
        elif probability > 0.65:
            recommendation = "рекомендуем закупить 5-10 экземпляров"
        else:
            recommendation = "рекомендуем закупить 3-5 экземпляров"
    else:
        recommendation = "достаточно 1-2 экземпляров"

    return {
        "prediction":     int(prediction),
        "label":          "популярная" if prediction == 1 else "непопулярная",
